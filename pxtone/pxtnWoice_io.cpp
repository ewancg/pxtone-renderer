...
pxtnERR pxtnWoice::io_matePCM_r(pxtnDescriptor *p_doc) {
  ...
  if (((int32_t)pcm.voice_flags) & PTV_VOICEFLAG_UNCOVERED)
...

  {
    ...
    res = p_vc->p_pcm->Create(pcm.ch, pcm.sps, pcm.bps,
                              pcm.data_size / (pcm.bps / 8 * pcm.ch));
...
pxtnERR pxtnWoice::io_mateOGGV_r(pxtnDescriptor *p_doc) {
  ...
  if (((int32_t)mate.voice_flags) & PTV_VOICEFLAG_UNCOVERED)
    return pxtnERR_fmt_unknown;
