...
pxtnService::~pxtnService() { _release(); }

pxtnERR pxtnService::init() { return _init(0, false); }
pxtnERR pxtnService::init_collage(int32_t fix_evels_num) {
...
bool pxtnService::Woice_Replace(int32_t old_place, int32_t new_place) {
  ...
  if (old_place < new_place) {
    for (int32_t w = old_place; w < new_place; w++) {
      if (_woices[w]) _woices[w] = _woices[w + 1];
    }
  } else {
    for (int32_t w = old_place; w > new_place; w--) {
      if (_woices[w]) _woices[w] = _woices[w - 1];
...
bool pxtnService::Unit_Replace(int32_t old_place, int32_t new_place) {
  ...
  if (old_place < new_place) {
    for (int32_t w = old_place; w < new_place; w++) {
      if (_units[w]) _units[w] = _units[w + 1];
    }
  } else {
    for (int32_t w = old_place; w > new_place; w--) {
      if (_units[w]) _units[w] = _units[w - 1];
...
pxtnERR pxtnService::_ReadVersion(void* desc, _enum_FMTVER* p_fmt_ver,
  ...
  if (!memcmp(version, _code_proj_x1x, _VERSIONSIZE)) {
    *p_fmt_ver = _enum_FMTVER_x1x;
    *p_exe_ver = 0;
    return pxtnOK;
  } else if (!memcmp(version, _code_proj_x2x, _VERSIONSIZE)) {
    *p_fmt_ver = _enum_FMTVER_x2x;
    *p_exe_ver = 0;
    return pxtnOK;
  } else if (!memcmp(version, _code_proj_x3x, _VERSIONSIZE)) {
    *p_fmt_ver = _enum_FMTVER_x3x;
  } else if (!memcmp(version, _code_proj_x4x, _VERSIONSIZE)) {
    *p_fmt_ver = _enum_FMTVER_x4x;
  } else if (!memcmp(version, _code_proj_v5, _VERSIONSIZE)) {
    *p_fmt_ver = _enum_FMTVER_v5;
  } else if (!memcmp(version, _code_tune_x2x, _VERSIONSIZE)) {
    *p_fmt_ver = _enum_FMTVER_x2x;
    *p_exe_ver = 0;
    return pxtnOK;
  } else if (!memcmp(version, _code_tune_x3x, _VERSIONSIZE)) {
    *p_fmt_ver = _enum_FMTVER_x3x;
  } else if (!memcmp(version, _code_tune_x4x, _VERSIONSIZE)) {
    *p_fmt_ver = _enum_FMTVER_x4x;
  } else if (!memcmp(version, _code_tune_v5, _VERSIONSIZE)) {
    *p_fmt_ver = _enum_FMTVER_v5;
  } else
...
pxtnERR pxtnService::_pre_count_event(void* desc, int32_t* p_count) {
  ...
  if (fmt_ver <= _enum_FMTVER_x3x)
    count += pxtnMAX_TUNEUNITSTRUCT *
             4;  // voice_no, group_no, key tuning, key event x3x
