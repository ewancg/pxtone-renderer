...
pxtnPulse_PCM::~pxtnPulse_PCM() { Release(); }
...
pxtnERR pxtnPulse_PCM::Create(int32_t ch, int32_t sps, int32_t bps,
                              int32_t sample_num) {
  Release();
...
pxtnERR pxtnPulse_PCM::read(pxtnDescriptor *doc) {
  ...
  char buf[16] = {0};
  uint32_t size = 0;
  WAVEFORMATCHUNK format = WAVEFORMATCHUNK();

  _p_smp = NULL;

  // 'RIFFxxxxWAVEfmt '
  if (!doc->r(buf, sizeof(char), 16)) {
    res = pxtnERR_desc_r;
    goto term;
  }
...
bool pxtnPulse_PCM::_Convert_ChannelNum(int32_t new_ch) {
  ...
  if (!(_p_smp = (uint8_t *)malloc(work_size))) {
    free(p_work);
    return false;
  }
...
pxtnERR pxtnPulse_PCM::Copy(pxtnPulse_PCM *p_dst) const {
  ...
  memcpy(p_dst->_p_smp, _p_smp,
         (_smp_head + _smp_body + _smp_tail) * _ch * _bps / 8);
  return pxtnOK;
}

bool pxtnPulse_PCM::Copy_(pxtnPulse_PCM *p_dst, int32_t start,
                          int32_t end) const {
  int32_t size, offset;
