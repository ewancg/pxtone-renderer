...
void pxtnPulse_Noise::set_smp_num_44k(int32_t num) { _smp_num_44k = num; }

int32_t pxtnPulse_Noise::get_unit_num() const { return _unit_num; }

int32_t pxtnPulse_Noise::get_smp_num_44k() const { return _smp_num_44k; }

float pxtnPulse_Noise::get_sec() const { return (float)_smp_num_44k / 44100; }
...
pxtnPulse_Noise::~pxtnPulse_Noise() { Release(); }
...
static bool _WriteOscillator(const pxNOISEDESIGN_OSCILLATOR *p_osc,
                             pxtnDescriptor *p_doc, int32_t *p_add) {
  int32_t work;
  work = (int32_t)p_osc->type;
  if (!p_doc->v_w_asfile(work, p_add)) return false;
  work = (int32_t)p_osc->b_rev;
  if (!p_doc->v_w_asfile(work, p_add)) return false;
  work = (int32_t)(p_osc->freq * 10);
  if (!p_doc->v_w_asfile(work, p_add)) return false;
  work = (int32_t)(p_osc->volume * 10);
  if (!p_doc->v_w_asfile(work, p_add)) return false;
  work = (int32_t)(p_osc->offset * 10);
  if (!p_doc->v_w_asfile(work, p_add)) return false;
  return true;
}

static pxtnERR _ReadOscillator(pxNOISEDESIGN_OSCILLATOR *p_osc,
                               pxtnDescriptor *p_doc) {
  int32_t work;
  ...
  for (u = 0; u < _unit_num; u++) {
    ...
    if (pU->bEnable) {
      ...
      if (flags & NOISEEDITFLAG_OSC_MAIN) {
        if (!_WriteOscillator(&pU->main, p_doc, &seek)) goto End;
      }
      if (flags & NOISEEDITFLAG_OSC_FREQ) {
        if (!_WriteOscillator(&pU->freq, p_doc, &seek)) goto End;
      }
      if (flags & NOISEEDITFLAG_OSC_VOLU) {
        if (!_WriteOscillator(&pU->volu, p_doc, &seek)) goto End;
      }
      unit_num++;
    }
  }
...
pxtnERR pxtnPulse_Noise::read(pxtnDescriptor *p_doc) {
  ...
  if (memcmp(code, _code, 8)) {
    res = pxtnERR_inv_code;
    goto term;
  }
  if (!p_doc->r(&ver, 4, 1)) {
    res = pxtnERR_desc_r;
  ...
  if (!pxtnMem_zero_alloc((void **)&_units,
                          sizeof(pxNOISEDESIGN_UNIT) * _unit_num)) {
    res = pxtnERR_memory;
    goto term;
  }
  ...
  for (int32_t u = 0; u < _unit_num; u++) {
    ...
    if (flags & NOISEEDITFLAG_ENVELOPE) {
      ...
      if (!pxtnMem_zero_alloc((void **)&pU->enves,
                              sizeof(pxtnPOINT) * pU->enve_num)) {
        res = pxtnERR_memory;
        goto term;
      }
...
bool pxtnPulse_Noise::Allocate(int32_t unit_num, int32_t envelope_num) {
  ...
  _unit_num = unit_num;
  if (!pxtnMem_zero_alloc((void **)&_units,
                          sizeof(pxNOISEDESIGN_UNIT) * unit_num))
    goto End;

  for (int32_t u = 0; u < unit_num; u++) {
    pxNOISEDESIGN_UNIT *p_unit = &_units[u];
    p_unit->enve_num = envelope_num;
    if (!pxtnMem_zero_alloc((void **)&p_unit->enves,
                            sizeof(pxtnPOINT) * p_unit->enve_num))
      goto End;
  }
  ...
  if (_unit_num) {
    ...
    for (int32_t u = 0; u < _unit_num; u++) {
      ...
      if (!(p_dst->_units[u].enves =
                (pxtnPOINT *)malloc(sizeof(pxtnPOINT) * enve_num)))
        goto End;
      for (int32_t e = 0; e < enve_num; e++)
...
static int32_t _CompareOsci(const pxNOISEDESIGN_OSCILLATOR *p_osc1,
                            const pxNOISEDESIGN_OSCILLATOR *p_osc2) {
