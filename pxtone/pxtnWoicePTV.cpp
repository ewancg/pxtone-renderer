...
static bool _Write_Wave(pxtnDescriptor *p_doc, const pxtnVOICEUNIT *p_vc,
                        int32_t *p_total) {
  bool b_ret = false;
  int32_t num, i, size;
  int8_t sc;
  ...
  switch (p_vc->type) {
    ...
    case pxtnVOICE_Coodinate:
      ...
      num = p_vc->wave.num;
      for (i = 0; i < num; i++) {
        uc = (int8_t)p_vc->wave.points[i].x;
        if (!p_doc->w_asfile(&uc, 1, 1)) goto End;
        (*p_total)++;
        sc = (int8_t)p_vc->wave.points[i].y;
        if (!p_doc->w_asfile(&sc, 1, 1)) goto End;
        (*p_total)++;
      }
      break;
    ...
    case pxtnVOICE_Noise:
      // This case didn't exist in the original source code
      break;
...
static bool _Write_Envelope(pxtnDescriptor *p_doc, const pxtnVOICEUNIT *p_vc,
                            int32_t *p_total) {
  bool b_ret = false;
  int32_t num, i;
  ...
  num = p_vc->envelope.head_num + p_vc->envelope.body_num +
        p_vc->envelope.tail_num;
  for (i = 0; i < num; i++) {
...
static pxtnERR _Read_Wave(pxtnDescriptor *p_doc, pxtnVOICEUNIT *p_vc) {
  ...
  switch (p_vc->type) {
    ...
    case pxtnVOICE_Coodinate:
      ...
      num = p_vc->wave.num;
      if (!pxtnMem_zero_alloc((void **)&p_vc->wave.points,
                              sizeof(pxtnPOINT) * num))
        return pxtnERR_memory;
      for (i = 0; i < num; i++) {
    ...
    // p_vc->sampring. (7)
    case pxtnVOICE_Sampling:
      return pxtnERR_fmt_unknown;  // un-support
      ...
      // size = ( p_vc->pcm.smp_head + p_vc->pcm.smp_body + p_vc->pcm.smp_tail )
      // * p_vc->pcm.ch * p_vc->pcm.bps / 8; if( !_malloc_zero( (void
      // **)&p_vc->pcm.p_smp,    size )          ) goto End; if( !p_doc->r(
      // p_vc->pcm.p_smp, 1, size ) ) goto End; break;

    default:
      return pxtnERR_ptv_no_supported;  // un-support
  }
...
static pxtnERR _Read_Envelope(pxtnDescriptor *p_doc, pxtnVOICEUNIT *p_vc) {
  ...
  num = p_vc->envelope.head_num + p_vc->envelope.body_num +
        p_vc->envelope.tail_num;
  if (!pxtnMem_zero_alloc((void **)&p_vc->envelope.points,
                          sizeof(pxtnPOINT) * num)) {
    res = pxtnERR_memory;
    goto term;
  }
...
pxtnERR pxtnWoice::PTV_Read(pxtnDescriptor *p_doc) {
  ...
  if (!p_doc->r(&version, sizeof(int32_t), 1)) {
    res = pxtnERR_desc_r;
    goto term;
  }
  ...
  if (!p_doc->r(&total, sizeof(int32_t), 1)) {
    res = pxtnERR_desc_r;
    goto term;
  }
  if (version > _version) {
  ...
  for (int32_t v = 0; v < _voice_num; v++) {
    ...
    if (!p_doc->v_r((int *)&p_vc->voice_flags)) {
      res = pxtnERR_desc_r;
      goto term;
    }
    if (!p_doc->v_r((int *)&p_vc->data_flags)) {
      res = pxtnERR_desc_r;
      goto term;
    }
