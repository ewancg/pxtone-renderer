...
pxtoneNoise::~pxtoneNoise() {
  if (_bldr) {
    delete (pxtnPulse_NoiseBuilder *)_bldr;
    _bldr = NULL;
  }
}
...
void pxtoneNoise::quality_get(int32_t *p_ch_num, int32_t *p_sps,
                              int32_t *p_bps) const {
...
bool pxtoneNoise::generate(pxtnDescriptor *p_doc, void **pp_buf,
                           int32_t *p_size) const {
  bool b_ret = false;
