...
pxtnWoice::~pxtnWoice() { Voice_Release(); }
...
#endif
    pxtnMem_free((void**)&p_vc->envelope.points);
    memset(&p_vc->envelope, 0, sizeof(pxtnVOICEENVELOPE));
    pxtnMem_free((void**)&p_vc->wave.points);
    memset(&p_vc->wave, 0, sizeof(pxtnVOICEWAVE));
  }
  if (p_vi) {
...
void pxtnWoice::Slim() {
  for (int32_t i = _voice_num - 1; i >= 0; i--) {
    ...
    if (_voices[i].type == pxtnVOICE_Coodinate && _voices[i].wave.num <= 1)
      b_remove = true;
...
bool pxtnWoice::Voice_Allocate(int32_t voice_num) {
  ...
  if (!pxtnMem_zero_alloc((void**)&_voices, sizeof(pxtnVOICEUNIT) * voice_num))
    goto End;
  if (!pxtnMem_zero_alloc((void**)&_voinsts,
                          sizeof(pxtnVOICEINSTANCE) * voice_num))
    goto End;
...
bool pxtnWoice::Copy(pxtnWoice* p_dst) const {
  ...
  for (v = 0; v < _voice_num; v++) {
    ...
    num = p_vc2->envelope.head_num + p_vc2->envelope.body_num +
          p_vc2->envelope.tail_num;
...
static void _UpdateWavePTV(pxtnVOICEUNIT* p_vc, pxtnVOICEINSTANCE* p_vi,
                           int32_t ch, int32_t bps) {
  double work, osc;
  int32_t long_;
  ...
  osci.ReadyGetSample(p_vc->wave.points, p_vc->wave.num, p_vc->volume,
                      p_vi->smp_body_w, p_vc->wave.reso);
  ...
  for (int32_t v = 0; v < _voice_num; v++) {
    ...
    switch (p_vc->type) {
      ...
      case pxtnVOICE_Coodinate: {
        ...
        if (!(p_vi->p_smp_w = (uint8_t*)malloc(size))) {
          res = pxtnERR_memory;
          goto term;
        }
      ...
      case pxtnVOICE_Noise: {
        ...
        if (!(p_pcm = ptn_bldr->BuildNoise(p_vc->p_ptn, ch, sps, bps))) {
          res = pxtnERR_ptn_build;
          goto term;
        }
...
pxtnERR pxtnWoice::Tone_Ready_envelope(int32_t sps) {
  ...
  for (int32_t v = 0; v < _voice_num; v++) {
    ...
    if (p_enve->head_num) {
      ...
      if (!pxtnMem_zero_alloc((void**)&p_vi->p_env, p_vi->env_size)) {
        res = pxtnERR_memory;
        goto term;
      }
      if (!pxtnMem_zero_alloc((void**)&p_point,
                              sizeof(pxtnPOINT) * p_enve->head_num)) {
        res = pxtnERR_memory;
        goto term;
      }
      ...
      for (int32_t s = 0; s < p_vi->env_size; s++) {
        ...
        if (e < head_num) {
          p_vi->p_env[s] =
              (uint8_t)(start.y + (p_point[e].y - start.y) * (s - start.x) /
                                      (p_point[e].x - start.x));
        } else {
    ...
    if (p_enve->tail_num) {
      p_vi->env_release = (int32_t)((double)p_enve->points[p_enve->head_num].x *
                                    sps / p_enve->fps);
    } else {
      p_vi->env_release = 0;
    }
...
term:
  ...
  if (res != pxtnOK) {
    for (int32_t v = 0; v < _voice_num; v++)
      pxtnMem_free((void**)&_voinsts[v].p_env);
  }

  return res;
}

pxtnERR pxtnWoice::Tone_Ready(const pxtnPulse_NoiseBuilder* ptn_bldr,
                              int32_t sps) {
  pxtnERR res = pxtnERR_VOID;
