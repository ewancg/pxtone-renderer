...
int32_t pxtnMaster::get_this_clock(int32_t meas, int32_t beat,
                                   int32_t clock) const {
...
void pxtnMaster::set_repeat_meas(int32_t meas) {
  if (meas < 0) meas = 0;
  _repeat_meas = meas;
}
void pxtnMaster::set_last_meas(int32_t meas) {
  if (meas < 0) meas = 0;
  _last_meas = meas;
}

void pxtnMaster::set_beat_clock(int32_t beat_clock) {
  ...
  for (e = 0; e < (int32_t)mast.event_num; e++) {
    ...
    switch (status) {
      case EVENTKIND_BEATCLOCK:
        beat_clock = volume;
        if (clock) return pxtnERR_desc_broken;
        break;
      case EVENTKIND_BEATTEMPO:
        memcpy(&beat_tempo, &volume, sizeof(float));
        if (clock) return pxtnERR_desc_broken;
        break;
      ...
      case EVENTKIND_REPEAT:
        repeat_clock = clock;
        if (volume) return pxtnERR_desc_broken;
        break;
      case EVENTKIND_LAST:
        last_clock = clock;
        if (volume) return pxtnERR_desc_broken;
